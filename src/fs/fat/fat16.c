...
#define ACTARUS_FAT16_SIGNATURE 0x29
#define ACTARUS_FAT16_FAT_ENTRY_SIZE 0x02
#define ACTARUS_FAT16_BAD_SECTOR 0xFF7
#define ACTARUS_FAT16_UNUSED 0x00
...
int fat16_get_total_items_for_directory(struct disk *disk, uint32_t directory_start_sector)
{
    ...
    if (diskstreamer_seek(stream, directory_start_pos) != ACTARUS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    while (1)
    {
        if (diskstreamer_read(stream, &item, sizeof(item)) != ACTARUS_ALL_OK)
        {
            res = -EIO;
            goto out;
        }
...
int fat16_get_root_directory(struct disk *disk, struct fat_private *fat_private, struct fat_directory *directory)
{
    ...
    struct disk_stream *stream = fat_private->directory_stream;
    if (diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != ACTARUS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }

    if (diskstreamer_read(stream, dir, root_dir_size) != ACTARUS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
int fat16_resolve(struct disk *disk)
{
    ...
    if (diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != ACTARUS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    if (fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != ACTARUS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
static int fat16_get_fat_entry(struct disk *disk, int cluster)
{
    ...
    res = diskstreamer_seek(stream, fat_table_position * (cluster * ACTARUS_FAT16_FAT_ENTRY_SIZE));
    if (res < 0)
    {
        goto out;
    }
...
static int fat16_get_cluster_for_offset(struct disk *disk, int starting_cluster, int offset)
{
    ...
    for (int i = 0; i < clusters_ahead; i++)
    {
        ...
        if (entry == ACTARUS_FAT16_BAD_SECTOR)
        {
            res = -EIO;
            goto out;
        }
...
struct fat_item *fat16_find_item_in_directory(struct disk *disk, struct fat_directory *directory, const char *name)
{
    ...
    char tmp_filename[ACTARUS_MAX_PATH];
...
int fat16_read(struct disk *disk, void *descriptor, uint32_t size, uint32_t nmemb, char *out_ptr)
{
    ...
    struct fat_directory_item *item = fat_descriptor->item->item; // we want the fat_directory item from fat_item, look structure to understand
    ...
    res = nmemb; // Res is equivalent to total_read
