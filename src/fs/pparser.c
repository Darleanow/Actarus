...
{
    int len = strnlen(filename, ACTARUS_MAX_PATH);
...
// TODO(Darleanow): Alloc only if something has been resolved
static const char *pathparser_get_path_part(const char **path)
{
    char *result_path_part = kzalloc(ACTARUS_MAX_PATH);
    int i = 0;
    ...
    if (**path == '/')
    {
        // Skip forward slash to avoid looping
        *path += 1;
    }
...
// todo(Darleanow): Implement relative paths
struct path_root* pathparser_parse(const char* path, const char* current_directory_path)
{
    ...
    if (strlen(path) > ACTARUS_MAX_PATH)
    {
        goto out;
    }
